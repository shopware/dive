...
import { DIVEEventExecutor } from '../EventExecutor.ts';
...
describe('dive/events/executor/DIVEEventExecutor', () => {
    let eventExecutor: DIVEEventExecutor<TestEvents>;

    beforeEach(() => {
        eventExecutor = new DIVEEventExecutor<TestEvents>();
    });

    describe('subscribe', () => {
        it('should subscribe to an event and return an unsubscribe function', () => {
            const listener = vi.fn();
            const unsubscribe = eventExecutor.subscribe('user-click', listener);
        ...
        it('should allow multiple subscribers to the same event', () => {
            ...
            const unsubscribe1 = eventExecutor.subscribe(
                'user-click',
                listener1,
            );
            const unsubscribe2 = eventExecutor.subscribe(
                'user-click',
                listener2,
            );
            const unsubscribe3 = eventExecutor.subscribe(
                'user-click',
                listener3,
            );
        ...
        it('should allow subscribing to different event types', () => {
            ...
            const unsubscribeClick = eventExecutor.subscribe(
                'user-click',
                clickListener,
            );
            const unsubscribeData = eventExecutor.subscribe(
                'data-loaded',
                dataListener,
            );
    ...
    describe('dispatch', () => {
        it('should call subscribed listeners when event is dispatched', () => {
            const listener = vi.fn();
            eventExecutor.subscribe('user-click', listener);

            const payload = { x: 100, y: 200 };
            // Using any to access protected method for testing
            (eventExecutor as any).dispatch('user-click', payload);
        ...
        it('should call all subscribed listeners for an event', () => {
            ...
            eventExecutor.subscribe('user-click', listener1);
            eventExecutor.subscribe('user-click', listener2);
            eventExecutor.subscribe('user-click', listener3);
        ...
        it('should not call listeners for different event types', () => {
            ...
            eventExecutor.subscribe('user-click', clickListener);
            eventExecutor.subscribe('data-loaded', dataListener);

            const clickPayload = { x: 10, y: 20 };
            (eventExecutor as any).dispatch('user-click', clickPayload);
        ...
        it('should handle events with no payload', () => {
            const listener = vi.fn();
            eventExecutor.subscribe('no-payload', listener);

            (eventExecutor as any).dispatch('no-payload');
        ...
        it('should handle dispatching to events with no listeners', () => {
            expect(() => {
                (eventExecutor as any).dispatch('user-click', { x: 1, y: 2 });
            }).not.toThrow();
        });

        it('should handle dispatching non-existent event types', () => {
            expect(() => {
                (eventExecutor as any).dispatch('non-existent-event', {});
            }).not.toThrow();
        });
        ...
        it('should call listeners in the order they were subscribed', () => {
            ...
            (eventExecutor as any).dispatch('user-click', { x: 0, y: 0 });
    ...
    describe('unsubscribe', () => {
        ...
        it('should return false when trying to unsubscribe from non-existent event type', () => {
            ...
            const fakeUnsubscribe = eventExecutor.subscribe(
                'data-loaded',
                vi.fn(),
            );
        ...
        it('should handle unsubscribing all listeners', () => {
            ...
            (eventExecutor as any).dispatch('user-click', { x: 5, y: 10 });
        ...
        it('should not affect listeners of other event types when unsubscribing', () => {
            ...
            (eventExecutor as any).dispatch('user-click', { x: 1, y: 2 });
            (eventExecutor as any).dispatch('data-loaded', {
                data: 'test',
                timestamp: 123,
            });
    ...
    describe('integration tests', () => {
        it('should handle complex subscription and dispatch patterns', () => {
            ...
            const unsubClick1 = eventExecutor.subscribe(
                'user-click',
                clickHandler1,
            );
            const unsubClick2 = eventExecutor.subscribe(
                'user-click',
                clickHandler2,
            );
            const unsubData = eventExecutor.subscribe(
                'data-loaded',
                dataHandler,
            );

            // Dispatch events
            (eventExecutor as any).dispatch('user-click', { x: 10, y: 20 });
            (eventExecutor as any).dispatch('data-loaded', {
                data: 'test-data',
                timestamp: 456,
            });
            ...
            // Dispatch again
            (eventExecutor as any).dispatch('user-click', { x: 30, y: 40 });
        ...
        it('should maintain correct listener references after multiple subscribe/unsubscribe operations', () => {
            ...
            const unsub1 = eventExecutor.subscribe('simple-event', listener);
            unsub1();

            const unsub2 = eventExecutor.subscribe('simple-event', listener);
            const unsub3 = eventExecutor.subscribe('simple-event', listener);

            unsub2();

            // Should still have one active subscription
            (eventExecutor as any).dispatch('simple-event', 'test-message');
            expect(listener).toHaveBeenCalledTimes(1);
