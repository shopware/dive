 ...
 *
 * Lights are plain `DIVENode`s carrying light components rather than classes of
 * their own, so `DIVENode` covers them -- and covers every other node a caller
 * composes from components.
 */
export type DIVESceneObject = DIVEModel | DIVEGroup | DIVEPrimitive | DIVENode;
