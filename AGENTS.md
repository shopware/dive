...
- Promise helpers live in `src/engine/promise/` subfolders: `abortable/AbortablePromise.ts` wraps an executor-backed cached promise with a per-run `AbortController`, and `deferred/DeferredPromise.ts` is directly awaitable/thenable with manual external `resolve()`/`reject()` and no `.promise` getter
