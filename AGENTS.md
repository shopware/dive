...
- `DIVEView` does not inject the clock into `DIVECanvasLifecycleManager`; only `DIVEView` itself is a `DIVETicker`, and the current `DIVE.startAsync()` sequence awaits `mainView.initAsync()` before `clock.startAsync()`, so tests should not expect clock startup while view init is still pending
