...
- Root Dependabot package-alert fixes may require Yarn v1 `resolutions` for transitive ranges that do not admit patched versions, such as `lodash` behind `@microsoft/api-extractor` and `minimatch` behind `@microsoft/api-extractor`; lockfile-only transitive entries within compatible ranges can be updated directly and verified with `yarn why`
- `src/plugins/state/package.json` currently has no dependencies; if Dependabot flags its stale `src/plugins/state/yarn.lock`, running `yarn install` inside `src/plugins/state/` should leave only the empty Yarn v1 lockfile header
